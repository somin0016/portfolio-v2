    ...
    const fetchSongs = async () => {
      ...
      try {
        const response = await fetch(
          'https://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=soeemthant&period=7day&limit=6&api_key=3186c83b5391d58fd507558c91f03192&format=json'
        );
        ...
        const updatedTracks = await Promise.all(tracks.map(async (track) => {
          ...
          if (trackInfoData?.track?.album) {
            ...
            const artistInfoResponse = await fetch(
              `https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${encodeURIComponent(track.artist.name)}&api_key=3186c83b5391d58fd507558c91f03192&format=json`
            );
            const artistInfoData = await artistInfoResponse.json();
            imageUrl = artistInfoData?.artist?.image?.find(img => img.size === 'large')?.["#text"];
          }
          return {
            ...track,
            image: imageUrl
      ...
      {isLoading ? (
        ...
        <>
          {songs.map((song, index) => (
            ...
            >
              <img src={song.image} className="rounded-md w-full" alt={song.name} />
              <div className="py-2">
                <h3 className="text-lg font-semibold">{song.name}</h3>
                <span className="text-sm inline-block">{song.artist.name}</span>
              </div>
            </div>
          ))}
        </>
